...
import com.company.wanbei.app.util.widget.genview.GenWheelText;
import com.company.wanbei.app.util.widget.genview.WheelGeneralAdapter;
import com.company.wanbei.app.util.widget.view.OnWheelChangedListener;
import com.company.wanbei.app.util.widget.view.OnWheelScrollListener;
 ...
 */
 abstract class BaseWheelPick
    ...
    protected void setWheelListener(WheelView1 wheelView1, Object[] data, boolean isCyclic) {
        WheelGeneralAdapter viewAdapter = new WheelGeneralAdapter(ctx, genView);
        if (data[0] instanceof Integer) {
            viewAdapter.setData(convertData(wheelView1, (Integer[]) data));
        } else {
            viewAdapter.setData(data);
        }
        wheelView1.setSelectTextColor(textColor, selectColor);
        wheelView1.setCyclic(isCyclic);
        wheelView1.setViewAdapter(viewAdapter);
        wheelView1.addChangingListener(this);
        wheelView1.addScrollingListener(this);
    }
