...
public class WorkNursePresenterImp implements WorkInterface.NursePresenterInterface{
    ...
    public void getNurseRecordList(String dateStr,String type) {
        ...
        map.put("type", type);
        RequestBody requestBody = HttpApi.packageParam(map);
        HttpApi.getPatientList(requestBody)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Subscriber<JSONNurseService>(){
                    ...
                    public void onNext(JSONNurseService result) {
                        ...
                        if ("0".equals(result.getCode())){
                            ...
                            view.reloadNurseRecord(result.getData());
                        }else {
                            view.showToast(result.getMsgBox());
                        }
                    }
...
//                                list.add(object);
//                            }
                            view.reloadCreateNurseList(result.getData());
                        }else {
                            view.showToast(result.getMsgBox());
                        }
                    }
    ...
    @Override
    public void UpdateServiceState(String id, String serviceState) {
        view.showDialog();
        JSONObject map = new JSONObject();
        map.put("id", id);
