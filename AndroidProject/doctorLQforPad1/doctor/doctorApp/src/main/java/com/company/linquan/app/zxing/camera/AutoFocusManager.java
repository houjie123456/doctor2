...
final class AutoFocusManager implements Camera.AutoFocusCallback {
    ...
    public boolean isSupportZoom(Camera camera) {
        ...
        Log.i(TAG, "--------------isSuppport:" + isSuppport);
        return isSuppport;
    }

    public void setZoom(Camera camera) {
        if (isSupportZoom(camera) && !zoomed) {
            try {
                Camera.Parameters params = camera.getParameters();
                final int MAX = params.getMaxZoom();
                if (MAX == 0) return;
                int zoomValue = params.getZoom();
                Log.i(TAG, "-----------------MAX:" + MAX + "   params : " + zoomValue);
                zoomValue = (MAX *1)/10;
                params.setZoom(zoomValue);
                camera.setParameters(params);
                zoomed = true;
                Log.i(TAG, "Is support Zoom " + params.isZoomSupported());
            } catch (Exception e) {
